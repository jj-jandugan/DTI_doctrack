...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    try {
        ...
        // --- NEW ACTION: UPLOAD SIGNED DOCUMENT & ROUTE TO DIVISION ---
        if ($_POST['action'] === 'upload_signed_and_send') {

            // 1. Handle Signed File Uploads
            if (!empty($_FILES['signed_document_files']['name'][0])) {
                $upload_dir = '../uploads/';
                if (!is_dir($upload_dir)) mkdir($upload_dir, 0777, true);

                foreach ($_FILES['signed_document_files']['tmp_name'] as $k => $tmp) {
                    if ($_FILES['signed_document_files']['error'][$k] === UPLOAD_ERR_OK) {
                        $fname = time() . '_SIGNED_VERSION_' . $k . '_' . $_FILES['signed_document_files']['name'][$k];
                        if (move_uploaded_file($tmp, $upload_dir . $fname)) {
                            // INSERT as signed version (is_signed = 1)
                            $pdo->prepare("INSERT INTO records_documentattachment (document_id, file_path, is_signed, uploaded_at) VALUES (?, ?, 1, NOW())")
                                ->execute([$doc_id, 'uploads/' . $fname]);
                        }
                    }
                }
            } else {
                throw new Exception("Please upload the signed version of the document before sending.");
            }

            // 2. IMPORTANT: Change status to 'APPROVED' Category
            // This is what makes the document move from the RO portal to the Division portal ("On My Desk")
            $stmtStatus = $pdo->query("SELECT id FROM records_status WHERE name = 'Routed' AND category = 'APPROVED' LIMIT 1");
            $status_id = $stmtStatus->fetchColumn();

            if (!$status_id) {
                $pdo->prepare("INSERT INTO records_status (name, category) VALUES ('Routed', 'APPROVED')")->execute();
                $status_id = $pdo->lastInsertId();
            }

            $pdo->prepare("UPDATE records_document SET status_id = ?, updated_at = NOW() WHERE id = ?")
                ->execute([$status_id, $doc_id]);

            // 3. Log History
            $pdo->prepare("INSERT INTO records_trackinghistory (action_taken, remarks, timestamp, acted_by_id, document_id)
                           VALUES ('SIGNED & ROUTED', 'Signed document uploaded by RO and officially forwarded to division personnel.', NOW(), ?, ?)")
                ->execute([$user_id, $doc_id]);

            $_SESSION['success_msg'] = "Signed document uploaded and successfully forwarded to Division.";
            $pdo->commit();
            header("Location: ../templates/ro/roIncoming.php");
            exit;
        }

        // --- ACTION: UPDATE DOCUMENT (Existing Logic) ---
            ...
            if (!empty($_FILES['document_files']['name'][0])) {
                ...
                foreach ($_FILES['document_files']['tmp_name'] as $k => $tmp) {
                    if ($_FILES['document_files']['error'][$k] === UPLOAD_ERR_OK) {
                        ...
                        if (move_uploaded_file($tmp, $upload_dir . $fname)) {
                            $pdo->prepare("INSERT INTO records_documentattachment (document_id, file_path, is_signed, uploaded_at) VALUES (?, ?, 0, NOW())")
                                ->execute([$doc_id, 'uploads/' . $fname]);
                        }
                    }
                }
        ...
        // --- ACTION: CANCEL DOCUMENT ---
            ...
            $_SESSION['success_msg'] = "Incoming Document successfully cancelled.";
        }
