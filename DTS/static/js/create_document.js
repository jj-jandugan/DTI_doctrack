...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // 1. Initialize the existing UploadQueue logic
    if (typeof UploadQueue !== 'undefined') {
        window.uploader = new UploadQueue("dropZone", "fileInput", "fileQueueDisplay", "dropZoneText");
    }

    // 2. Validate on Form Submit
    form.addEventListener('submit', (e) => {
        // Check if the uploader queue is empty
        ...
        if (!hasFiles) {
            ...
            // Show error message and highlight the box
    ...
    // Reset error when user starts dragging or clicks
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (newDocModalEl && cancelConfirmModalEl) {
        // Use a static modal instance (no fade for speed as requested before)
        const encoderModal = new bootstrap.Modal(newDocModalEl, { backdrop: 'static' });

        // Logic to bring back the main modal if they click "Keep Editing"
        cancelConfirmModalEl.addEventListener('hidden.bs.modal', function () {
            // If they didn't just refresh/discard, show the main modal again
            if (document.activeElement.innerText !== "Yes, Discard") {
                encoderModal.show();
            }
        });
    }
