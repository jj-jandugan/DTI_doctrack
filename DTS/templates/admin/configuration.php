    ...
    try {
        ...
        elseif (str_starts_with($action, 'delete_')) {
            ...
            if ($action === 'delete_classification')
                $table = 'records_classification';
            if ($action === 'delete_type')
                $table = 'records_documenttype';
            if ($action === 'delete_division')
                $table = 'records_division';
            if ($action === 'delete_status')
                $table = 'records_status';
...
?> <div class="dashboard-inner p-4">
    <div class="mb-4">
        <h2 class="text-dark fw-bold mb-0">System Configurations</h2>
        <p class="text-secondary">Manage the dropdown options and reference data used throughout the system.</p>
    </div>
    <?php if ($success_msg): ?>
    ...
    <ul class="nav nav-tabs" id="configTabs" role="tablist">
        <li class="nav-item">
            <button class="nav-link <?= $active_tab == 'classifications' ? 'active' : '' ?>" data-bs-toggle="tab"
                data-bs-target="#classifications">Classifications</button>
        </li>
        <li class="nav-item">
            <button class="nav-link <?= $active_tab == 'doctypes' ? 'active' : '' ?>" data-bs-toggle="tab"
                data-bs-target="#doctypes">Document Types</button>
        </li>
        <li class="nav-item">
            <button class="nav-link <?= $active_tab == 'divisions' ? 'active' : '' ?>" data-bs-toggle="tab"
                data-bs-target="#divisions">Divisions</button>
        </li>
        <li class="nav-item">
            <button class="nav-link <?= $active_tab == 'statuses' ? 'active' : '' ?>" data-bs-toggle="tab"
                data-bs-target="#statuses">Statuses</button>
        </li>
    </ul>
    <div class="tab-content">
            ...
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <form method="POST" action="configuration.php" class="d-flex gap-2">
                    ...
                    <input type="text" name="name" class="form-control form-control-sm"
                        placeholder="New Classification..." required style="width: 250px;">
                            ...
                            <tr>
                                ...
                                <td>
                                    <button type="button" class="btn btn-sm btn-outline-secondary" data-bs-toggle="modal"
                                        data-bs-target="#editClassModal" data-id="<?= $class['id'] ?>"
                                        data-name="<?= htmlspecialchars($class['name']) ?>"><i
                                            class="fa-solid fa-pen"></i></button>
                                    <button type="button" class="btn btn-sm btn-outline-danger" data-bs-toggle="modal"
                                        data-bs-target="#deleteConfirmModal" data-id="<?= $class['id'] ?>"
                                        data-name="<?= htmlspecialchars($class['name']) ?>"
                                        data-action="delete_classification" data-tab="classifications"><i
        ...
        <div class="tab-pane fade <?= $active_tab == 'doctypes' ? 'show active' : '' ?>" id="doctypes">
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <form method="POST" action="configuration.php" class="d-flex gap-2">
                    ...
                    <input type="text" name="name" class="form-control form-control-sm"
                        placeholder="New Document Type..." required style="width: 250px;">
                    <button type="submit" class="btn btn-blue btn-sm"><i class="fa-solid fa-plus me-1"></i> Add</button>
                </form>
            </div>
            <div class="table-container p-0">
                    ...
                    <tbody>
                        <?php foreach ($doc_types as $type): ?>
                            <tr>
                                ...
                                <td>
                                    <button type="button" class="btn btn-sm btn-outline-secondary" data-bs-toggle="modal"
                                        data-bs-target="#editTypeModal" data-id="<?= $type['id'] ?>"
                                        data-name="<?= htmlspecialchars($type['name']) ?>"><i
                                            class="fa-solid fa-pen"></i></button>
                                    <button type="button" class="btn btn-sm btn-outline-danger" data-bs-toggle="modal"
                                        data-bs-target="#deleteConfirmModal" data-id="<?= $type['id'] ?>"
                                        data-name="<?= htmlspecialchars($type['name']) ?>" data-action="delete_type"
                                        data-tab="doctypes"><i class="fa-solid fa-trash"></i></button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
        ...
        <div class="tab-pane fade <?= $active_tab == 'divisions' ? 'show active' : '' ?>" id="divisions">
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <button type="button" class="btn btn-blue btn-sm" data-bs-toggle="modal"
                    data-bs-target="#addDivisionModal"><i class="fa-solid fa-plus me-1"></i> Add Division</button>
            </div>
            <div class="table-container p-0">
                <table class="data-table">
                    <thead>
                    ...
                    <tbody>
                        <?php foreach ($divisions as $div): ?>
                            <tr>
                                <td>#<?= $div['id'] ?></td>
                                <td><span
                                        class="badge bg-light text-dark border px-2 py-1"><?= htmlspecialchars($div['abbreviation']) ?></span>
                                </td>
                                <td class="fw-bold text-dark"><?= htmlspecialchars($div['name']) ?></td>
                                <td>
                                    <button type="button" class="btn btn-sm btn-outline-secondary" data-bs-toggle="modal"
                                        data-bs-target="#editDivisionModal" data-id="<?= $div['id'] ?>"
                                        data-abbr="<?= htmlspecialchars($div['abbreviation']) ?>"
                                        data-name="<?= htmlspecialchars($div['name']) ?>"><i
                                            class="fa-solid fa-pen"></i></button>
                                    <button type="button" class="btn btn-sm btn-outline-danger" data-bs-toggle="modal"
                                        data-bs-target="#deleteConfirmModal" data-id="<?= $div['id'] ?>"
                                        data-name="<?= htmlspecialchars($div['abbreviation']) ?>"
                                        data-action="delete_division" data-tab="divisions"><i
                                            class="fa-solid fa-trash"></i></button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
        ...
        <div class="tab-pane fade <?= $active_tab == 'statuses' ? 'show active' : '' ?>" id="statuses">
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <button type="button" class="btn btn-blue btn-sm" data-bs-toggle="modal"
                    data-bs-target="#addStatusModal"><i class="fa-solid fa-plus me-1"></i> Add Status</button>
            </div>
            <div class="table-container p-0">
                <table class="data-table">
                    <thead>
                        ...
                        <?php foreach ($statuses as $status): ?>
                            <tr>
                                ...
                                <td><span class="text-muted"
                                        style="font-size: 0.85rem; text-transform: uppercase;"><?= htmlspecialchars($status['category']) ?></span>
                                </td>
                                <td>
                                    <button type="button" class="btn btn-sm btn-outline-secondary" data-bs-toggle="modal"
                                        data-bs-target="#editStatusModal" data-id="<?= $status['id'] ?>"
                                        data-name="<?= htmlspecialchars($status['name']) ?>"
                                        data-category="<?= htmlspecialchars($status['category']) ?>"><i
                                            class="fa-solid fa-pen"></i></button>
                                    <button type="button" class="btn btn-sm btn-outline-danger" data-bs-toggle="modal"
                                        data-bs-target="#deleteConfirmModal" data-id="<?= $status['id'] ?>"
                                        data-name="<?= htmlspecialchars($status['name']) ?>" data-action="delete_status"
                                        data-tab="statuses"><i class="fa-solid fa-trash"></i></button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content custom-modal">
            <div class="modal-header border-0 pb-0 pt-4 px-4">
                <h5 class="fw-bold text-dark"><i class="fa-solid fa-building me-2" style="color: #263D81;"></i> Add
                    Division</h5><button type="button" class="btn-close shadow-none" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body px-4 py-4">
                <form method="POST"><input type="hidden" name="action" value="add_division"><input type="hidden"
                        name="active_tab" value="divisions">
                    <div class="mb-3"><label class="form-label modal-label">Abbreviation *</label><input type="text"
                            name="abbreviation" class="form-control custom-input" required></div>
                    <div class="mb-4"><label class="form-label modal-label">Full Name *</label><input type="text"
                            name="name" class="form-control custom-input" required></div>
                    <div class="d-flex justify-content-end gap-2 border-top pt-3"><button type="button"
                            class="btn btn-cancel" data-bs-dismiss="modal">Cancel</button><button type="submit"
                            class="btn btn-blue px-4">Save</button></div>
                </form>
            </div>
        </div>
    </div>
...
<div class="modal fade" id="addStatusModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content custom-modal">
            <div class="modal-header border-0 pb-0 pt-4 px-4">
                <h5 class="fw-bold text-dark"><i class="fa-solid fa-tag me-2" style="color: #263D81;"></i> Add Status
                </h5><button type="button" class="btn-close shadow-none" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body px-4 py-4">
                <form method="POST"><input type="hidden" name="action" value="add_status"><input type="hidden"
                        name="active_tab" value="statuses">
                    <div class="mb-3"><label class="form-label modal-label">Name *</label><input type="text" name="name"
                            class="form-control custom-input" required></div>
                    <div class="mb-4"><label class="form-label modal-label">Category *</label><select name="category"
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content custom-modal">
            <div class="modal-body p-4">
                ...
                <form method="POST"><input type="hidden" name="action" value="edit_classification"><input type="hidden"
                        name="active_tab" value="classifications"><input type="hidden" name="id" id="editClassId">
                    <input type="text" name="name" id="editClassName" class="form-control custom-input mb-4" required>
                    <div class="d-flex justify-content-end gap-2"><button type="button" class="btn btn-cancel"
                            data-bs-dismiss="modal">Cancel</button><button type="submit"
                            class="btn btn-blue px-4">Update</button></div>
                </form>
            </div>
        </div>
    </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content custom-modal">
            <div class="modal-body p-4">
                ...
                <form method="POST"><input type="hidden" name="action" value="edit_type"><input type="hidden"
                        name="active_tab" value="doctypes"><input type="hidden" name="id" id="editTypeId">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content custom-modal">
            <div class="modal-body p-4">
                ...
                <form method="POST"><input type="hidden" name="action" value="edit_division"><input type="hidden"
                        name="active_tab" value="divisions"><input type="hidden" name="id" id="editDivId">
                    <div class="mb-3"><label class="form-label">Abbreviation</label><input type="text"
                            name="abbreviation" id="editDivAbbr" class="form-control custom-input" required></div>
                    <div class="mb-4"><label class="form-label">Full Name</label><input type="text" name="name"
                            id="editDivName" class="form-control custom-input" required></div>
                    <div class="d-flex justify-content-end gap-2"><button type="button" class="btn btn-cancel"
                            data-bs-dismiss="modal">Cancel</button><button type="submit"
                            class="btn btn-blue px-4">Update</button></div>
                </form>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content custom-modal">
            <div class="modal-body p-4">
                ...
                <form method="POST"><input type="hidden" name="action" value="edit_status"><input type="hidden"
                        name="active_tab" value="statuses"><input type="hidden" name="id" id="editStatusId">
                    <div class="mb-3"><label class="form-label">Status Name</label><input type="text" name="name"
                            id="editStatusName" class="form-control custom-input" required></div>
                    <div class="mb-4"><label class="form-label">Category</label><select name="category"
                            id="editStatusCat" class="form-select custom-input" required>
                            <option value="ONGOING">ONGOING</option>
                            <option value="FOR-APPROVAL">FOR APPROVAL</option>
                            <option value="CLOSED">CLOSED</option>
                        </select></div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content custom-modal border-danger">
            ...
            <div class="modal-body px-4 py-4 text-center">
                <p class="mb-4">Are you sure you want to permanently delete <strong id="delete_item_name"
...
// Add dynamic JS to fill modals
$extra_js = "
<script>
document.addEventListener('DOMContentLoaded', function() {
    // Edit Modals
    document.getElementById('editClassModal')?.addEventListener('show.bs.modal', e => {
    ...
    document.getElementById('editTypeModal')?.addEventListener('show.bs.modal', e => {
    ...
    document.getElementById('editDivisionModal')?.addEventListener('show.bs.modal', e => {
    ...
    document.getElementById('editStatusModal')?.addEventListener('show.bs.modal', e => {
    ...
    document.getElementById('deleteConfirmModal')?.addEventListener('show.bs.modal', e => {
