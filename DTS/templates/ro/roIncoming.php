...
?> <div class="dashboard-inner p-4">
    ...
    <div class="d-flex align-items-center justify-content-between gap-3 mb-4">
        <button type="button" class="btn btn-new-document flex-shrink-0" data-bs-toggle="modal"
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <?php foreach ($my_encoded_docs as $doc): ?>
                            <tr class="doc-row clickable-row"
                                onclick="window.location.href='roEditDocu.php?id=<?= $doc['id'] ?>'">
                                ...
                                <td class="search-target text-success fw-bold"><i
                                        class="fa-solid fa-arrow-right-to-bracket me-1"></i>
                                    <?= htmlspecialchars($doc['destination_name'] ?? '') ?></td>
                                <td class="fw-bold text-dark search-target text-truncate" style="max-width: 200px;">
                                    <?= htmlspecialchars($doc['subject']) ?>
                                    <br><span
                                        class="text-muted fw-normal small"><?= date('M d, Y', strtotime($doc['created_at'])) ?></span>
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content custom-modal">
            <div class="modal-header border-0 pb-0 pt-4 px-4">
                <h5 class="modal-title fw-bold text-dark"><i class="fa-solid fa-file-import me-2 text-success"></i>
                    Encode Incoming Document</h5>
                <button type="button" class="btn-close shadow-none" data-bs-toggle="modal"
                    data-bs-target="#cancelConfirmModal"></button>
            </div>
            <div class="modal-body px-4 py-4">
                <form method="POST" action="../../controllers/ro_controller.php" enctype="multipart/form-data">
                    ...
                    <div class="row g-4 mb-4">
                        ...
                        <div class="col-md-4"><label class="modal-label">Due Date (Optional)</label><input type="date"
                                name="due_date" class="form-control custom-input"></div>
                    </div>
                    <div class="row g-4 mb-4 mt-4 border p-3 rounded bg-light">
                        <div class="col-md-6"><label class="modal-label">Origin (Office/Agency) *</label>
                            <input list="officeOptions" name="origin_office"
                                class="form-control custom-input border-warning" required autocomplete="off">
                            <datalist id="officeOptions">
                                <?php foreach ($pdo->query("SELECT name FROM records_origin")->fetchAll(PDO::FETCH_COLUMN) as $o): ?>
                                    <option value="<?= htmlspecialchars($o) ?>"><?php endforeach; ?>
                            </datalist>
                        </div>
                        <div class="col-md-6"><label class="modal-label">Contact Person</label><input type="text"
                                name="sender_name" class="form-control custom-input"></div>
                    </div>
                    <div class="mb-4"><label class="modal-label">Subject *</label><textarea name="subject"
                            class="form-control custom-input" rows="2" required></textarea></div>
                    <div class="border rounded p-4 mb-4 bg-light">
                        <label class="fw-bold text-success mb-3"><i class="fa-solid fa-sitemap me-1"></i> Address
                            (Internal Destination) *</label>
                        <select class="form-select custom-input mb-3 border-success" name="route_type" id="routeType"
                            required>
                    ...
                    <div class="row g-4 mb-4">
                        ...
                        <div class="col-md-6"><label class="modal-label">Particulars</label><input type="text"
                                name="particulars" class="form-control custom-input"></div>
                    </div>
                    <div class="mb-4">
        ...
        <div class="modal-content custom-modal">
            <div class="modal-body text-center p-4">
                ...
                <div class="d-flex flex-column gap-2 mt-4">
                    <button type="button" class="btn btn-warning fw-bold w-100" onclick="location.reload()">Yes,
                        Discard</button>
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content custom-modal" style="border: 2px solid #eab308;">
            <div class="modal-body text-center p-4">
                ...
                <p class="text-muted" style="font-size: 0.9rem;">You are about to close the encoder. Any information
                ...
                <div class="d-flex flex-column gap-2 mt-4">
                    ...
                    <button type="button" class="btn btn-warning fw-bold text-dark w-100"
                        onclick="window.location.reload()"> Yes, Discard </button>
                    <!-- This button simply closes this confirmation warning -->
                    <button type="button" class="btn btn-light w-100 border" data-bs-dismiss="modal"> No, Keep Editing
                    </button>
                </div>
            </div>
        </div>
