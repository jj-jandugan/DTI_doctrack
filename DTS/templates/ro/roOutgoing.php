            ...
            <table class="data-table" id="outgoingTable">
                ...
                <tbody>
                    <?php if (empty($outgoing_docs)): ?>
                        <tr>
                            <td colspan="7" class="text-center text-muted py-5">No approved documents waiting for dispatch.
                            </td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($outgoing_docs as $doc): ?>
                            <tr class="doc-row clickable-row"
                                onclick="window.location.href='roViewOutgoing.php?id=<?= $doc['id'] ?>'">
                                ...
                                <td class="fw-bold text-dark text-truncate search-target" style="max-width: 250px;">
                                    <?= htmlspecialchars($doc['subject']) ?></td>
                                <td class="search-target text-success fw-bold">
                                    <?= htmlspecialchars($doc['address_name'] ?? 'External Agency') ?></td>
