...
if (!$document_id) {
    header("Location: roIncoming.php");
    exit;
}
...
?> <div class="dashboard-inner p-4">
    <div class="mb-4">
        <div class="mb-4">
            <a href="roIncoming.php" id="backToQueue" class="text-decoration-none text-secondary fw-bold"
                style="font-size: 0.9rem;">
                <i class="fa-solid fa-arrow-left me-1"></i> Back to Incoming Queue </a>
        </div>
        <div class="edit-container">
            <div class="d-flex justify-content-between align-items-center mb-4 pb-3 border-bottom">
                <h3 class="fw-bold text-dark mb-0"><i class="fa-solid fa-pen-to-square text-primary me-2"></i> Edit
                    Record</h3>
            ...
            <form id="editDocForm" method="POST" action="../../controllers/ro_controller.php?id=<?= $document_id ?>"
                ...
                <div class="row g-4 mb-4">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="date" name="due_date" class="form-control custom-input"
                            value="<?= $doc['due_date'] ? date('Y-m-d', strtotime($doc['due_date'])) : '' ?>">
                    </div>
                </div>
                ...
                <div class="row g-4 mb-4">
                    <div class="col-md-6">
                        ...
                        <input list="officeOptions" name="origin_office" class="form-control custom-input"
                            value="<?= htmlspecialchars($doc['origin_name']) ?>" required>
                        <datalist id="officeOptions">
                            <?php foreach ($saved_origins as $org): ?>
                                <option value="<?= htmlspecialchars($org) ?>"><?php endforeach; ?>
                        </datalist>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label modal-label">Sender Name</label>
                        <input type="text" name="sender_name" class="form-control custom-input"
                            value="<?= htmlspecialchars($doc['sender'] ?? '') ?>">
                    </div>
                </div>
                <div class="mb-4">
                    <label class="form-label modal-label">Subject *</label>
                    <textarea name="subject" class="form-control custom-input" rows="2"
                        required><?= htmlspecialchars($doc['subject']) ?></textarea>
                </div>
                ...
                <div class="border rounded p-4 mb-4"
                    ...
                    <div id="block-division" class="<?= ($doc['route_type'] === 'division') ? '' : 'd-none' ?>">
                        <div class="row g-3">
                            ...
                            <div class="col-md-7">
                                <div id="routeUsersContainer" class="checkbox-container custom-input"
                ...
                <div class="row g-3 mb-4">
                    <div class="col-md-6">
                        <label class="form-label modal-label text-primary"><i class="fa-solid fa-signature me-1"></i>
                            Route to Signatory *</label>
                    ...
                    <?php if (!empty($attachments)): ?>
                        <div class="mb-3">
                            <?php foreach ($attachments as $att): ?>
                                <div class="existing-file d-flex justify-content-between align-items-center bg-white"
                                    id="att-row-<?= $att['id'] ?>"
                                    style="border: 1px solid #e2e8f0; border-radius: 6px; padding: 12px 16px; margin-bottom: 10px;">
                                    <span class="text-dark"
                                        style="font-size: 0.9rem;"><?= htmlspecialchars(basename($att['file_path'])) ?></span>
                ...
                <div class="d-flex justify-content-end gap-3 pt-4 border-top mt-5">
                    ...
                    <button type="submit" id="submitBtn" class="btn btn-blue px-4 fw-bold"
                        style="background-color: #1d4ed8; border-radius: 6px;">Save Changes</button>
                </div>
            </form>
        </div>
    </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <div class="modal-content custom-modal" style="border: 2px solid #eab308;">
                <div class="modal-body text-center p-4">
                    ...
                    <p class="text-muted" style="font-size: 0.9rem;">Any unsaved data will be lost. Are you sure you
                        want to discard your edits?</p>
                    <div class="d-flex flex-column gap-2 mt-4">
                        <button type="button" class="btn btn-warning fw-bold text-dark w-100"
                            onclick="window.location.href='roIncoming.php'">Yes, Discard</button>
            ...
            <div class="modal-content custom-modal" style="border: 2px solid #1d4ed8;">
                <div class="modal-body text-center p-4">
                    ...
                    <div class="d-flex flex-column gap-2 mt-4">
                        ...
                        <button type="button" id="confirmSaveBtn" class="btn btn-blue fw-bold w-100"
                            style="background-color: #1d4ed8; color: #fff;"> Confirm Save </button>
