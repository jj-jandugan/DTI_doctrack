...
?> <div class="dashboard-inner p-4">
    ...
    <a href="<?= htmlspecialchars($from_page) ?>" class="btn btn-link text-secondary mb-4 p-0 text-decoration-none">
        <i class="fa-solid fa-arrow-left me-1"></i> Back to Queue </a>
        ...
        <div class="row">
                        ...
                        <?php else: ?>
                            <?php foreach ($attachments as $file): ?>
                                <a href="<?= BASE_URL . $file['file_path'] ?>" target="_blank"
                                    class="d-flex align-items-center p-3 border rounded mb-2 text-decoration-none bg-white attachment-item">
            ...
            <div class="col-md-4 border-start ps-4">
                <div class="mb-4">
                    ...
                    <div class="fw-bold text-dark"><?= htmlspecialchars($doc['origin_name'] ?? 'Internal Division') ?>
                    </div>
                </div>
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <div
                        class="<?= ($doc['due_date'] && strtotime($doc['due_date']) < time()) ? 'text-danger' : 'text-dark' ?> fw-bold">
                        <i class="fa-regular fa-calendar me-1"></i>
                        <?= $doc['due_date'] ? date('M d, Y', strtotime($doc['due_date'])) : 'No Deadline' ?>
                    </div>
                </div>
                <div class="pt-4 border-top">
                    ...
                    <form method="POST" action="../../controllers/signatory.php">
                        ...
                        <button type="submit" name="action" value="approve_document"
                            class="btn btn-success w-100 py-3 fw-bold mb-3 shadow-sm">
                            <i class="fa-solid fa-file-signature me-2"></i> Approve & Sign </button>
                        <button type="submit" name="action" value="reject_document"
                            class="btn btn-outline-danger w-100 py-2 fw-bold"
                            onclick="return confirm('Are you sure you want to REJECT this document?')">
                            <i class="fa-solid fa-xmark me-2"></i> Reject Document </button>
                    </form>
                </div>
            </div>
        </div>
