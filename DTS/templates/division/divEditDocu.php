...
?> <div class="dashboard-inner p-4">
    <div class="mb-4">
        <a href="divOutgoing.php" id="backToQueue" class="text-decoration-none text-secondary fw-bold"
            style="font-size: 0.9rem;">
            <i class="fa-solid fa-arrow-left me-1"></i> Back to Outgoing Queue </a>
    </div>
    <div class="edit-container">
        <div class="d-flex justify-content-between align-items-center mb-4 pb-3 border-bottom">
            <h3 class="fw-bold text-dark mb-0"><i class="fa-solid fa-pen-to-square text-primary me-2"></i> Manage
                Document</h3>
        ...
        <?php if ($is_locked): ?>
            <div class="alert alert-warning border-warning mb-4" style="background-color: #fffbeb;">
                <i class="fa-solid fa-lock me-2 text-warning"></i> <strong>Document Locked.</strong> This document has
                already been approved or dispatched and can no longer be edited.
            </div>
        <?php endif; ?>
        <?php if ($success_msg): ?>
            <div class="alert alert-success"><?= htmlspecialchars($success_msg) ?></div><?php endif; ?>
        <?php if ($error_msg): ?>
            <div class="alert alert-danger"><?= htmlspecialchars($error_msg) ?></div><?php endif; ?>
        <form id="editDocForm" method="POST" action="../../controllers/editDocument.php" enctype="multipart/form-data">
            ...
            <div class="row g-4 mb-4">
                <div class="col-md-4">
                    ...
                    <select class="form-select custom-input <?= $is_locked ? 'bg-light' : '' ?>" name="classification"
                ...
                <div class="col-md-4">
                    ...
                    <select class="form-select custom-input <?= $is_locked ? 'bg-light' : '' ?>" name="document_type"
                        required <?= $is_locked ? 'disabled' : '' ?>>
                        <option value="">Select...</option>
                ...
                <div class="col-md-4">
                    ...
                    <input type="date" name="due_date"
                        class="form-control custom-input <?= $is_locked ? 'bg-light' : '' ?>"
                        value="<?= $doc['due_date'] ? date('Y-m-d', strtotime($doc['due_date'])) : '' ?>" <?= $is_locked ? 'readonly' : '' ?>>
                </div>
            </div>
            <div class="mb-4">
                <label class="form-label modal-label">Subject *</label>
                <textarea name="subject" class="form-control custom-input <?= $is_locked ? 'bg-light' : '' ?>" rows="2"
                    required <?= $is_locked ? 'readonly' : '' ?>><?= htmlspecialchars($doc['subject']) ?></textarea>
            </div>
            <div class="mb-4">
                <label class="form-label modal-label">Particulars</label>
                <textarea name="particulars" class="form-control custom-input <?= $is_locked ? 'bg-light' : '' ?>"
                    rows="3" <?= $is_locked ? 'readonly' : '' ?>><?= htmlspecialchars($doc['particulars'] ?? '') ?></textarea>
            </div>
            <div class="row g-4 mb-4">
                <div class="col-md-6">
                    <label class="form-label modal-label text-primary"><i class="fa-solid fa-signature me-1"></i> Route
                        to Signatory (Optional)</label>
                    <select class="form-select custom-input <?= $is_locked ? 'bg-light' : '' ?>" name="signatory"
                        <?= $is_locked ? 'disabled' : '' ?>>
                        <option value="">None...</option>
                        <?php foreach ($signatories as $sig): ?>
                            <option value="<?= $sig['id'] ?>" <?= ($sig['id'] == $doc['signatory_id']) ? 'selected' : '' ?>>
                                <?= htmlspecialchars($sig['first_name'] . ' ' . $sig['last_name']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-md-6">
                    <label class="form-label modal-label">Destination / Routing Notes</label>
                    <input type="text" name="sender_notes"
                        class="form-control custom-input <?= $is_locked ? 'bg-light' : '' ?>"
                        value="<?= htmlspecialchars($doc['sender'] ?? '') ?>" <?= $is_locked ? 'readonly' : '' ?>>
                </div>
            </div>
            <div class="border rounded p-4 mb-4" style="background-color: #f8fafc; border-color: #e2e8f0 !important;">
                ...
                <select class="form-select custom-input mb-3 <?= $is_locked ? 'bg-light' : '' ?>" name="route_type"
                    id="routeType" required <?= $is_locked ? 'disabled' : '' ?>>
                    <option value="division" <?= ($doc['route_type'] === 'division') ? 'selected' : '' ?>>Division /
                        Specific Personnel</option>
                    <option value="group" <?= ($doc['route_type'] === 'group') ? 'selected' : '' ?>>Distribution Group
                    </option>
                </select>
                <div id="block-division" class="<?= ($doc['route_type'] === 'division') ? '' : 'd-none' ?>">
                    <div class="row g-3">
                        <div class="col-md-5">
                            <select class="form-select custom-input <?= $is_locked ? 'bg-light' : '' ?>"
                                id="routeDivision" name="route_division" <?= $is_locked ? 'disabled' : '' ?>>
                                <option value="">Select Division...</option>
                                <?php foreach ($divisions as $div): ?>
                                    <option value="<?= $div['id'] ?>" <?= ($div['id'] == $current_div_id) ? 'selected' : '' ?>>
                                        <?= htmlspecialchars($div['name']) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="col-md-7">
                            <div id="routeUsersContainer"
                                class="checkbox-container custom-input <?= $is_locked ? 'bg-light' : '' ?>"
                ...
                <div id="block-group" class="<?= ($doc['route_type'] === 'group') ? '' : 'd-none' ?>">
                    <select class="form-select custom-input <?= $is_locked ? 'bg-light' : '' ?>" name="route_group"
                        ...
                        <?php foreach ($groups as $grp): ?>
                            <option value="<?= $grp['id'] ?>" <?= ($doc['route_type'] === 'group' && !empty($current_recipients) ? 'selected' : '') ?>>
                                <?= htmlspecialchars($grp['group_name']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
            <div class="mb-4 border rounded p-4" style="background-color: #fff;">
                <label class="form-label modal-label"><i class="fa-solid fa-paperclip me-2 text-primary"></i>
                    Attachments</label>
                <?php if (!empty($attachments)): ?>
                    <div class="mb-3 mt-2">
                        <?php foreach ($attachments as $att): ?>
                            <div class="existing-file d-flex justify-content-between align-items-center bg-light"
                                id="att-row-<?= $att['id'] ?>"
                                style="border: 1px solid #e2e8f0; border-radius: 6px; padding: 12px 16px; margin-bottom: 10px;">
                                <div>
                                    <i class="fa-regular fa-file-pdf text-danger me-2"></i>
                                    <a href="<?= BASE_URL . $att['file_path'] ?>" target="_blank"
                                        class="text-dark fw-bold text-decoration-none"
                                        style="font-size: 0.9rem;"><?= htmlspecialchars(basename($att['file_path'])) ?></a>
                                </div>
                                <?php if (!$is_locked): ?>
                                    <button type="button" class="btn-remove-file p-0 m-0 text-danger bg-transparent border-0"
                                        onclick="queueFileRemoval(event, <?= $att['id'] ?>)"
                                        style="font-size: 1.2rem; line-height: 1;" title="Remove Attachment">&times;</button>
                                <?php endif; ?>
                            </div>
                        <?php endforeach; ?>
                    </div>
                ...
                <?php if (!$is_locked): ?>
                    <div class="drag-drop-box mt-3" id="dropZone">
                        <div class="upload-content text-center w-100">
                            ...
                            <input type="file" id="fileInput" name="document_files[]" multiple
            ...
            <?php if (!$is_locked): ?>
                <div class="d-flex justify-content-end gap-3 pt-4 border-top mt-5">
                    <button type="button" class="btn btn-secondary px-4 fw-bold" style="border-radius: 6px;"
                        data-bs-toggle="modal" data-bs-target="#cancelEditModal">Cancel</button>
                    <button type="button" class="btn btn-blue px-4 fw-bold"
                        style="background-color: #1d4ed8; border-radius: 6px;" data-bs-toggle="modal"
                        data-bs-target="#saveConfirmModal">Save Changes</button>
                </div>
            <?php endif; ?>
        </form>
    </div>
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content custom-modal" style="border: 2px solid #eab308;">
            <div class="modal-body text-center p-4">
                ...
                <p class="text-muted" style="font-size: 0.9rem;">Any unsaved data will be lost. Are you sure you want to
                    discard your edits?</p>
                <div class="d-flex flex-column gap-2 mt-4">
                    <button type="button" class="btn btn-warning fw-bold text-dark w-100"
                        onclick="window.location.href='divOutgoing.php'">Yes, Discard</button>
                    <button type="button" class="btn btn-light w-100 border" data-bs-dismiss="modal">No, Keep
                        Editing</button>
                </div>
            </div>
        </div>
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content custom-modal" style="border: 2px solid #1d4ed8;">
            <div class="modal-body text-center p-4">
                ...
                <p class="text-muted" style="font-size: 0.9rem;">Are you sure you want to apply these updates to the
                    document record?</p>
                <div class="d-flex flex-column gap-2 mt-4">
                    <button type="button" onclick="document.getElementById('editDocForm').submit();"
                        class="btn btn-blue fw-bold w-100" style="background-color: #1d4ed8; color: #fff;"> Confirm Save
                    </button>
                    <button type="button" class="btn btn-light w-100 border" data-bs-dismiss="modal"> Review Edits
                    </button>
                </div>
            </div>
        </div>
