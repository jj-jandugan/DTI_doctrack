    ...
    <div class="edit-container">
        ...
        <?php if ($is_locked): ?>
            <div class="alert alert-warning border-warning mb-4" style="background-color: #fffbeb;">
                <i class="fa-solid fa-lock me-2 text-warning"></i> <strong>Document Locked.</strong> This document has already been approved or dispatched and can no longer be edited.
            </div>
        <?php endif; ?>
        ...
        <form id="editDocForm" method="POST" action="../../controllers/editDocument.php" enctype="multipart/form-data">
            ...
            <div class="row g-4 mb-4">
                <div class="col-md-4">
                    ...
                    <select class="form-select custom-input <?= $is_locked ? 'bg-light' : '' ?>" name="classification" required <?= $is_locked ? 'disabled' : '' ?>>
            ...
            <div class="mb-4">
                ...
                <textarea name="particulars" class="form-control custom-input <?= $is_locked ? 'bg-light' : '' ?>" rows="3" <?= $is_locked ? 'readonly' : '' ?>><?= htmlspecialchars($doc['particulars'] ?? '') ?></textarea>
            </div>

            <div class="row g-4 mb-4">
                <div class="col-md-6">
                    <label class="form-label modal-label text-primary"><i class="fa-solid fa-signature me-1"></i> Route to Signatory (Optional)</label>
                    <select class="form-select custom-input <?= $is_locked ? 'bg-light' : '' ?>" name="signatory" <?= $is_locked ? 'disabled' : '' ?>>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="sender_notes" class="form-control custom-input <?= $is_locked ? 'bg-light' : '' ?>" value="<?= htmlspecialchars($doc['sender'] ?? '') ?>" <?= $is_locked ? 'readonly' : '' ?>>
                </div>
            </div>
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content custom-modal" style="border: 2px solid #1d4ed8;">
            <div class="modal-body text-center p-4">
                ...
                <div class="d-flex flex-column gap-2 mt-4">
                    <button type="button" onclick="document.getElementById('editDocForm').submit();" class="btn btn-blue fw-bold w-100" style="background-color: #1d4ed8; color: #fff;">
                        Confirm Save
                    </button>
