...
// Fetch Attachments, History, and Recipients
...
// THE FIX: Properly pull recipient names depending on the routing type!
if (in_array($doc['route_type'], ['outside_dti', 'within_dti'])) {

    // Fetch the specific contact names you typed from the external recipients table
    ...
    // Internal Routing (Division/Group Users)
    if (!empty($recipients)) {
...
$level = 1; // Default to Encoded
$is_rejected = false;

// 1. Check if Rejected or Cancelled
if (strpos($combined_status, 'REJECT') !== false || strpos($combined_status, 'CANCEL') !== false) {
    $is_rejected = true;
}

// 2. Determine Highest Level Reached
...
// 3. Assign CSS Classes based on Level
$step1 = 'completed';
...
// 4. Calculate Colored Progress Line Width
$progress_width = 0;
if ($level == 2) $progress_width = 25;
elseif ($level == 3) $progress_width = 50;
elseif ($level == 4) $progress_width = 75;
...
<div class="dashboard-inner p-3">
    ...
    <div class="detail-card border rounded shadow-sm bg-white p-4">
            ...
            <div class="timeline-container">
                ...
                <?php else: ?>
                    <?php foreach ($history_logs as $index => $log):
                        ...
                        elseif(strtoupper($log['action_taken']) === 'APPROVED' || strpos(strtoupper($log['action_taken']), 'CLOSED') !== false || strpos(strtoupper($log['action_taken']), 'DISPATCHED') !== false) { $icon = 'fa-check'; $bg = '#10b981'; }
        ...
        <h6 class="border-bottom pb-2 mb-4 text-secondary mt-4" style="font-size: 0.9rem;">
            <i class="fa-solid fa-circle-info me-2"></i>Document Details
        </h6>
        ...
        <?php if ($is_incoming): ?>
        <h6 class="border-bottom pb-2 mb-2 text-secondary mt-4" style="font-size: 0.9rem;">
