...
$outgoing_docs = $docManager->getOutgoingForApproval($user_id);
// Fetch the specific recipients mapped to each document
$all_recipients = $docManager->getRecipientsByDocument();

// Link existing modular CSS files
$extra_css = '
...
?> <div class="dashboard-inner p-4">
    ...
    <div class="filter-bar d-flex flex-wrap align-items-center gap-3 mb-4 w-100">
        <div class="input-group search-container m-0 shadow-sm"
            style="flex: 1 1 280px; max-width: 400px; border-radius: 6px;">
        ...
        <div class="d-flex flex-wrap align-items-center gap-3 flex-grow-1">
            ...
            <div class="d-flex align-items-center gap-2 border px-3 py-1 rounded bg-white shadow-sm"
                style="height: 38px;">
            ...
            <table class="data-table" id="outgoingTable">
                ...
                <tbody>
                        ...
                        <tr class="doc-row clickable-row"
                            ...
                            <td class="deadline-target align-middle">
                                <?php if ($doc['due_date']): ?>
                                    ...
                                    <?php if ($days < 0): ?>
                                        <span
                                            class="badge bg-danger bg-opacity-10 text-danger border border-danger rounded-pill px-2 py-1 mb-1"
                                            style="font-size:0.7rem;">
                                            <i class="fa-solid fa-circle-exclamation me-1"></i>Overdue </span>
                                        <div class="small fw-bold text-danger"><?= date('M d, Y', strtotime($doc['due_date'])) ?>
                                        </div>
                                    <?php elseif ($days === 0): ?>
                                        <span
                                            class="badge bg-warning bg-opacity-10 text-warning border border-warning rounded-pill px-2 py-1 mb-1"
                                            style="font-size:0.7rem;">
                                            <i class="fa-solid fa-clock me-1"></i>Due Today </span>
                                        <div class="small fw-bold text-dark"><?= date('M d, Y', strtotime($doc['due_date'])) ?>
                                        </div>
                                    <?php else: ?>
                            ...
                            <td class="search-target">
                                ...
                                <div class="text-muted mt-1" style="font-size: 0.75rem; line-height: 1.2;">
                                    ...
                                    if (in_array($doc['route_type'], ['outside_dti', 'within_dti'])) {
                                        echo htmlspecialchars($doc['sender'] ?? 'No specific personnel assigned');
                                    }
                                    // Otherwise, it is an internal route, fetch from $all_recipients array
                                    else {
                            ...
                            <td>
                                <div class="creator-cell d-flex align-items-center m-0 p-0 bg-transparent border-0">
                                    <div class="creator-avatar d-flex justify-content-center align-items-center bg-secondary text-white rounded-circle me-2 flex-shrink-0"
                                        style="width: 32px; height: 32px;">
                                        <i class="fa-solid fa-user" style="font-size: 0.85rem;"></i>
                                    </div>
                                    <div class="creator-info d-flex flex-column">
                                        <span class="creator-name fw-bold text-dark"
                                        ...
                                        <span class="text-muted" style="font-size: 0.75rem; line-height: 1.2;">
                                            <?= htmlspecialchars($doc['c_division'] ?? 'Division') ?>
                                        </span>
                                        <span
                    ...
                    <?php if (empty($outgoing_docs)): ?>
                        <tr>
                            <td colspan="7" class="text-center text-muted py-5">No outgoing documents waiting for signature.
                            </td>
                        </tr>
                    <?php endif; ?>
                </tbody>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        if (typeof TableFilter !== 'undefined') {
            ...
            console.error("filters.js failed to load.");
        }
    });
</script>
