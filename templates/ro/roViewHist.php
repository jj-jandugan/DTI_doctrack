...
$stmt = $pdo->prepare("
    SELECT d.*, c.name as class_name, t.name as type_name, s.name as status_name, s.category as status_category,
           ...
           divi.name as c_division, sig.first_name as sig_fname, sig.last_name as sig_lname
...
if ($is_rejected) { $level = 5; }
elseif (strpos($combined_status, 'CLOSE') !== false || strpos($combined_status, 'DISPATCHED') !== false || strpos($combined_status, 'RECEIVED') !== false) { $level = 5; }
elseif (strpos($combined_status, 'ROUTED') !== false || $doc['status_category'] === 'APPROVED') { $level = 4; }
elseif (strpos($combined_status, 'FOR DISPATCH') !== false || strpos($combined_status, 'FOR-DISPATCH') !== false || strpos($combined_status, 'SENT') !== false) { $level = 4; }
...
<style>
    ...
    .track-item { display: flex; position: relative; padding-bottom: 20px; }
    .track-item::before {
...
<div class="dashboard-inner p-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <a href="roHistory.php" class="text-decoration-none text-muted small fw-bold">
                <i class="fa-solid fa-arrow-left me-1"></i> Back to History
            </a>
            <h2 class="fw-bold text-dark mt-2 mb-0"><?= htmlspecialchars($doc['dts_no']) ?></h2>
        </div>
        <form action="../../controllers/reuseDocument.php" method="POST" class="m-0">
            <input type="hidden" name="doc_id" value="<?= $doc['id'] ?>">
            <button type="submit" class="btn btn-outline-primary btn-sm fw-bold shadow-sm px-3" onclick="return confirm('Reuse this record?')">
                <i class="fa-solid fa-copy me-1"></i> Reuse Record
            </button>
        </form>
    </div>
    ...
    <div class="bg-white border rounded-4 p-4 mb-4 shadow-sm">
        ...
        <div class="position-relative mt-4 mb-2">
            <div style="position: absolute; top: 22px; left: 10%; right: 10%; height: 4px; background: #e2e8f0; z-index: 1;"></div>
            <div style="position: absolute; top: 22px; left: 10%; width: calc(80% * <?= $progress_width ?> / 100); height: 4px; background: <?= $is_rejected ? '#dc3545' : '#10b981' ?>; z-index: 2; transition: width 0.5s ease;"></div>
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
            <div class="bg-white rounded-4 shadow-sm p-4 border h-100">
                <div class="d-flex justify-content-between align-items-start mb-4">
                    <div>
                        <h5 class="fw-bold text-dark mb-1"><i class="fa-solid fa-circle-info me-2 text-primary"></i> Document Details</h5>
                        <p class="text-muted small mb-0">Historical metadata and routing information.</p>
                    </div>
                    <span class="badge rounded-pill bg-secondary px-3 py-2"><?= htmlspecialchars($doc['status_name']) ?></span>
                </div>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        <div class="info-box">
                            ...
                            <div class="info-value text-primary"><?= htmlspecialchars($doc['class_name'] ?? 'N/A') ?></div>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="info-box">
                    ...
                    <div class="col-md-6">
                        <div class="info-box">
                            ...
                            <div class="info-value text-danger"><?= !empty($doc['due_date']) ? date('M d, Y', strtotime($doc['due_date'])) : 'None' ?></div>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="info-box">
                            <div class="info-label">Particulars</div>
                            <div class="info-value fw-normal" style="white-space: pre-wrap;"><?= htmlspecialchars($doc['particulars'] ?? 'No particulars provided.') ?></div>
                        </div>
                    </div>
                    <div class="col-12">
                        <div class="info-box">
                ...
                <div class="row g-3">
                    <div class="col-12">
                        <div class="info-box">
                            ...
                            <div class="info-value"><?= htmlspecialchars(trim(($doc['sig_fname'] ?? '') . ' ' . ($doc['sig_lname'] ?? ''))) ?: 'None' ?></div>
                        </div>
                    </div>

                    <div class="col-12 mt-4">
                        <h6 class="info-label text-muted border-bottom pb-1 mb-3">Address</h6>
                        <div class="row g-3">
                            <div class="col-md-6">
                                <div class="info-box">
                                    <div class="info-label">Division / Group</div>
                                    <div class="info-value small"><?= htmlspecialchars($doc['address_name'] ?? 'N/A') ?></div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="info-box">
                ...
                <!-- REVISED ORIGIN INFORMATION: Hidden if origin_name is empty -->
                        ...
                        ?>
                            <div class="info-box d-flex align-items-center">
                                ...
                                <div class="flex-grow-1">
                                    ...
                                    <div class="text-muted" style="font-size: 0.7rem;">Click icon to view attachment</div>
                                </div>
        ...
        <div class="col-lg-4">
            <div class="bg-white rounded-4 shadow-sm p-4 border h-100">
                <h5 class="fw-bold text-dark mb-1"><i class="fa-solid fa-clock-rotate-left me-2 text-secondary"></i> Tracking History</h5>
                <p class="text-muted small mb-4">Activity log for this document.</p>

                <div class="tracking-timeline">
                    <?php if (empty($history_logs)): ?>
                        <div class="alert alert-light border small text-muted">No history recorded yet.</div>
                    <?php else: ?>
                        ...
                        ?>
                            <div class="track-item <?= $entry_class ?>">
                                ...
                                <div class="history-box">
                                    <div class="fw-bold text-dark small mb-1"><?= htmlspecialchars($log['action_taken']) ?></div>
                                    <div class="text-muted mb-2" style="font-size: 0.7rem;"><?= date('M d, Y - h:i A', strtotime($log['timestamp'])) ?></div>
                                    ...
                                    <div class="pt-2 border-top mt-2 d-flex align-items-center">
                                        ...
                                        <div style="font-size: 0.75rem;">
                                            ...
                                            <div class="text-muted"><?= htmlspecialchars($doc['c_division']) ?></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <?php if (count($history_logs) > 4): ?>
                            <div class="text-center mt-3">
                                <button id="toggle-history-btn" class="btn btn-sm btn-link text-decoration-none fw-bold">See More History</button>
                            </div>
                        <?php endif; ?>
                    <?php endif; ?>
                </div>
    ...
    <!-- Footer Metadata -->
         <div class="mt-4 pt-4 border-top d-flex align-items-center">
            ...
            <div class="text-dark mb-0">
                <div class="fw-bold text-dark small">Created By by: <span class="text-primary"><?= htmlspecialchars($doc['c_fname'].' '.$doc['c_lname']) ?></span></div>
