...
$doc_id = $_GET['id'] ?? null;
if (!$doc_id) { header("Location: roHistory.php"); exit; }
...
if (!$doc) { header("Location: roHistory.php"); exit; }
...
// NEW FIX: Properly pull recipient names depending on the routing type!
...
if (in_array($doc['route_type'], ['outside_dti', 'within_dti'])) {
    // Fetch the specific contact names from the external recipients table
    ...
    if (!empty($valid_contacts)) {
        $dest_names_html = implode('<br>', array_map('htmlspecialchars', $valid_contacts));
    }
} else {
    // Internal Routing
    if (!empty($recipients)) {
...
<div class="dashboard-inner p-3">
    <div class="mb-3 d-flex justify-content-between align-items-center">
        <a href="roHistory.php" class="btn-back text-decoration-none text-secondary"><i class="fa-solid fa-chevron-left me-2"></i><span class="fw-bold">Back to History</span></a>
        <form action="../../controllers/reuseDocument.php" method="POST" class="m-0 p-0" onsubmit="return confirm('Reuse this record? A new draft will be created.');">
            <input type="hidden" name="doc_id" value="<?= $doc['id'] ?>">
            <button type="submit" class="btn btn-sm btn-primary rounded text-white px-3"><i class="fa-solid fa-copy me-1"></i> Reuse Record</button>
        </form>
    </div>
    ...
    <div class="detail-card border rounded shadow-sm bg-white p-4">
        ...
        <div class="bg-light border rounded p-4 mb-4 shadow-sm">
            ...
            <div class="position-relative mt-4 mb-2">
                ...
                <div class="visual-stepper d-flex justify-content-between position-relative" style="z-index: 3; padding: 0;">
                    ...
                    <div class="step <?= $step4 ?>"><div class="circle"><i class="fa-solid fa-boxes-packing"></i></div><div class="label">Waiting Dispatch</div></div>
                    <div class="step <?= $step5 ?>"><div class="circle"><?php if($is_rejected): ?><i class="fa-solid fa-xmark"></i><?php else: ?><i class="fa-solid fa-paper-plane"></i><?php endif; ?></div><div class="label"><?= ($is_rejected) ? 'Rejected' : 'Dispatch' ?></div></div>
                </div>
            </div>
        </div>
        ...
        <div class="row g-3 mb-3">
            <div class="col-md-3"><div class="data-group"><label class="text-muted text-uppercase small">Deadline</label><p class="data-value text-dark"><?= !empty($doc['due_date']) ? date('M d, Y', strtotime($doc['due_date'])) : 'None' ?></p></div></div>
            <div class="col-md-3"><div class="data-group"><label class="text-muted text-uppercase small">Classification</label><p class="data-value text-dark"><?= htmlspecialchars($doc['class_name'] ?? 'N/A') ?></p></div></div>
            <div class="col-md-3"><div class="data-group"><label class="text-muted text-uppercase small">Document Type</label><p class="data-value text-dark"><?= htmlspecialchars($doc['type_name'] ?? 'N/A') ?></p></div></div>
            <div class="col-md-3"><div class="data-group"><label class="text-muted text-uppercase small">Signatory</label><p class="data-value text-dark"><?= htmlspecialchars(trim(($doc['sig_fname'] ?? '') . ' ' . ($doc['sig_lname'] ?? ''))) ?></p></div></div>
        </div>
        ...
        <div class="row g-3 mb-3">
            <div class="col-md-6"><div class="data-group"><label class="text-muted text-uppercase small">Origin Office</label><div class="data-value textarea-style bg-light text-dark py-2 px-3 border-0"><?= htmlspecialchars($doc['origin_name'] ?? 'External Office') ?></div></div></div>
            <div class="col-md-6"><div class="data-group"><label class="text-muted text-uppercase small">Sender Name</label><div class="data-value textarea-style bg-light text-dark py-2 px-3 border-0"><?= htmlspecialchars($doc['sender'] ?? 'N/A') ?></div></div></div>
        </div>
        <div class="row g-3 mb-3">
            <div class="col-md-6"><div class="data-group"><label class="text-muted text-uppercase small">Destination / Agency</label><div class="data-value textarea-style bg-light text-dark py-2 px-3 border-0"><?= htmlspecialchars($doc['address_name'] ?? 'Internal Routing') ?></div></div></div>
            <div class="col-md-6"><div class="data-group"><label class="text-muted text-uppercase small">Recipient Name(s)</label><div class="data-value textarea-style bg-light text-dark py-2 px-3 border-0"><?= $dest_names_html ?></div></div></div>
        </div>

        <div class="row g-3 mb-3">
            <div class="col-12"><div class="data-group"><label class="text-muted text-uppercase small">Particulars</label><div class="data-value textarea-style bg-light text-dark py-2 px-3 border-0"><?= nl2br(htmlspecialchars($doc['particulars'] ?? 'No particulars provided.')) ?></div></div></div>
        </div>
