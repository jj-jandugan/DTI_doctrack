                ...
                <?php if(!empty($attachments)): ?>
                    <div class="mb-3">
                        <?php foreach($attachments as $att): ?>
                            <div class="existing-file d-flex justify-content-between align-items-center bg-white shadow-sm" id="att-row-<?= $att['id'] ?>" style="border: 1px solid #e2e8f0; border-radius: 6px; padding: 12px 16px; margin-bottom: 10px;">
                                <span class="text-dark" style="font-size: 0.9rem;"><i class="fa-solid fa-paperclip text-secondary me-2"></i><?= htmlspecialchars(basename($att['file_path'])) ?></span>
                                <button type="button" class="btn-remove-file p-0 m-0 text-danger bg-transparent border-0" onclick="queueFileRemoval(event, <?= $att['id'] ?>)" title="Remove Attachment" style="font-size: 1.2rem; line-height: 1;">&times;</button>
                            </div>
                        <?php endforeach; ?>
                    </div>
                <?php endif; ?>
